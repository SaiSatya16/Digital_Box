...
class _ChatScreenState extends State<ChatScreen> with SingleTickerProviderStateMixin {
  ...
  void _downloadAttachment(String attachmentUrl) async {
    ...
    if (status.isGranted) {
      ...
      // Handle the download progress or completion if needed
      FlutterDownloader.registerCallback((id, status, progress) {
        // Update the UI or show download progress if desired
      });
    } else {
  ...
  void _connectToServer() {
    socket = IO.io('http://digital-box-socket.ap-south-1.elasticbeanstalk.com');
    socket.on('connect', (_) {
  ...
  void _generateSummary() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://51smg9qx-5000.inc1.devtunnels.ms/summarize/${widget.complaintId}'),
      );
  ...
  void _closeTicket() async {
    ...
    if (confirmClose == true) {
      final response = await http.put(
        Uri.parse('https://51smg9qx-5000.inc1.devtunnels.ms/ticket/${widget.ticketId}'),
